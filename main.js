...
async function loadModularFooter() {
    const footerContainer = document.getElementById('footerPlaceholder');
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.warn('Modular footer fetch returned status:', response.status);
        }
    } catch (err) {
        console.warn('Note: Dynamic footer inclusion via fetch is active. Running on a local server (e.g. Live Server) is recommended for AJAX includes.', err);
    }
}
